...
class Mdemailrapor extends CI_Model{
    public function email($where){
        $this->db->select('orangtua.email_orangtua,orangtua.nama_orangtua,user.nama_depan,user.nama_belakang');
        $this->db->from('orangtua');
        $this->db->join('siswa', 'orangtua.id_orangtua = siswa.id_orangtua','inner');
