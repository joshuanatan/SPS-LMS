...
class Mdorangtua extends CI_Model{
    ...
    public function selectortuanak($where){
        return $this->db->query("select * from siswa inner join orangtua on orangtua.id_orangtua = siswa.id_orangtua inner join user on user.id_user = siswa.id_user inner join siswa_angkatan on siswa_angkatan.id_siswa = siswa.id_siswa where siswa_angkatan.id_siswa_angkatan = ".$where." and siswa.monitor_ortu = 0");
        //select oraang tua dari siswa yang dimasukin dimana orangtuanya mau monitor
        /*
        query = select * from siswa inner join orangtua on orangtua.id_orangtua = siswa.id_orangtua inner join user on user.id_user = siswa.id_user inner join siswa_angkatan on siswa_angkatan.id_siswa = siswa.id_siswa where siswa_angkatan.id_siswa_angkatan = 1
        */
    }
